...
export default function XetHocBa_HinhThuc2({
  user,
  ...
  const handleChange = (value) => {
    ...
    for (let i = 0; i < 3; i++) {
      if (major[i] === "nk") {
        ...
        currentYourScore +=
          (scores.tongLop12["ly"] +
            scores.tongLop12["hoa"] +
            scores.tongLop12["sinh"]) /
          3;
      } else if (major[i] === "khxh") {
        currentYourScore +=
          (scores.tongLop12["su"] +
            scores.tongLop12["dia"] +
            scores.tongLop12["gdcd"]) /
          3;
      } else {
        currentYourScore += scores.tongLop12[major[i]];
      }
    }
  ...
  return (
    <div>
      <div className="text-xl font-bold text-center">
        PHƯƠNG THỨC XÉT HỌC BẠ (XÉT ĐIỂM TRUNG BÌNH CÁC MÔN (ĐIỂM TỔNG KẾT CẢ
        NĂM) LỚP 12)
      </div>
      <Divider />
